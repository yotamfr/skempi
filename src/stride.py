...
def delta_sasa(pdb, chainA, chainB):
    args = ['%s/stride/stride' % ROOT, '%s/data/pdbs/%s.pdb' % (ROOT, pdb), '-r%s%s' % (chainA, chainB)]
    proc0 = subprocess.Popen(args, stdout=subprocess.PIPE)
    df_complex = parse_stride(proc0.stdout)
    args = ['%s/stride/stride' % ROOT, '%s/data/pdbs/%s.pdb' % (ROOT, pdb), '-r%s' % (chainA,)]
    proc1 = subprocess.Popen(args, stdout=subprocess.PIPE)
    df_A = parse_stride(proc1.stdout)
    args = ['%s/stride/stride' % ROOT, '%s/data/pdbs/%s.pdb' % (ROOT, pdb), '-r%s' % (chainB,)]
    proc2 = subprocess.Popen(args, stdout=subprocess.PIPE)
    df_B = parse_stride(proc2.stdout)
    assert np.all(np.array(list(df_A["Res"]) +
                           list(df_B["Res"])) == np.array(list(df_complex["Res"])))
    assert np.any(np.array(list(df_A["ASA"]) +
                           list(df_B["ASA"])) != np.array(list(df_complex["ASA"])))
...
if __name__ == "__main__":
    if not os.path.exists('%s/data/stride' % ROOT):
        os.makedirs('%s/data/stride' % ROOT)
