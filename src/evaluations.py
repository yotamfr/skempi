...
def run_cv_test(dataset, get_regressor, save_prefix=None, normalize=0, groups=DIMER_GROUPS):
    ...
    for i, (indx_trn, indx_tst) in enumerate(indexes):
        ...
        y_pred = regressor.predict(X_trn)
        cor_trn, _ = pearsonr(y_trn, y_pred)
        y_pred = regressor.predict(X_tst)
        cor_tst, _ = pearsonr(y_tst, y_pred)
        print("G%d" % (i + 1,), "%.3f" % cor_trn, "%.3f" % cor_tst)
        results["Protein"].extend(names[indx_tst])
        results["Mutations"].extend(mutations[indx_tst])
        results["DDG_PRED"].extend(y_pred)
